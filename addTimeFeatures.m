...
%and the hour is a magnitude value with 0 as midnight and 23 as 11PM.
   ...
   % features: hour, sunday, ..., saturday, jan, ..., dec, 2003, ..., 2015
   features = zeros(length(epochTimes), 1 + 7 + 12 + 13);
   for i = 1:length(epochTimes)
      timeStruct = localtime(epochTimes(i));
      features(i, 1) = timeStruct.hour;            % assign hour
      features(i, 2 + timeStruct.wday) = 1;        % assign day
      features(i, 9 + timeStruct.mon) = 1;         % assign month
      features(i, 21 + timeStruct.year - 103) = 1; % assign month
   end
end
